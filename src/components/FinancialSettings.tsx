...
type Props = {
  ...
  onSaveGeminiPreference: (previewTransactions: boolean) => Promise<void>
...
export default function FinancialSettings(props: Props) {
  const { workspace, workspaces, settings, accountEmail, onCreateWorkspace, onSelectWorkspace, onClearWorkspace, onDeleteWorkspace, onSaveRate, onSavePreferences, onSaveGeminiPreference, onResetPassword, onSignOut } = props
  ...
  const [previewTransactions, setPreviewTransactions] = useState(settings.geminiTransactionPreview ?? true)
  ...
  useEffect(() => { setRate(String(settings.exchangeRate)); setRemindersEnabled(settings.subscriptionRemindersEnabled ?? true); setReminderDays((settings.subscriptionReminderDays ?? [7, 3, 1]).join(', ')); setBrowserEnabled(settings.browserNotifications ?? false); setPreviewTransactions(settings.geminiTransactionPreview ?? true) }, [settings])
  ...
  const saveAiSettings = async (event: React.FormEvent) => {
    ...
    try {
      ...
      } else if (modelToSave !== (gemini?.model || DEFAULT_GEMINI_MODEL)) {
        const status = await saveGeminiModel(modelToSave)
        setGemini(status)
        if (status.model) setSelectedModel(sanitizeModel(status.model))
        setSaved('gemini_model')
      }
      await onSaveGeminiPreference(previewTransactions)
      setSaved(geminiKey.trim() ? 'gemini' : modelToSave !== (gemini?.model || DEFAULT_GEMINI_MODEL) ? 'gemini_model' : 'gemini_behavior')
    } catch (reason) {
  ...
  const cards = [
    ...
    { id: 'gemini' as const, title: 'Gemini AI', copy: 'Secure API key, model selection, voice, conversation memory, and action behavior.', status: gemini?.configured ? gemini.model : DEFAULT_GEMINI_MODEL, Icon: Chat },
  ...
  return <Glass className="module-view financial-settings settings-detail">
    ...
    {section === 'gemini' && <form className="settings-focus-card gemini-focus" onSubmit={saveAiSettings}><div className="settings-focus-heading"><span><Chat set="curved" size={23} /></span><div><small>AI CONNECTION</small><h1>GEMINI AI</h1><p>Connect your Gemini API key and choose how the assistant handles complete transaction requests.</p></div></div><div className={`gemini-status ${gemini?.configured ? 'connected' : ''}`}><span><i />{checkingGemini ? 'CHECKING CONNECTION' : gemini?.configured ? 'CONNECTED' : 'NOT CONNECTED'}</span><strong>{gemini?.model || selectedModel}</strong><small>{gemini?.configured ? 'Your encrypted key is available only to the authenticated server function.' : 'Paste a key below. It is verified before encrypted storage in Supabase Vault.'}</small></div><label><span>AI MODEL {loadingModels ? '· FETCHING YOUR KEY MODELS…' : ''}</span><select value={selectedModel} onChange={(event) => { setSelectedModel(event.target.value); setSaved(null) }}>{modelOptions.map((item) => <option key={item.id} value={item.id}>{item.name || item.id} {item.description ? `— ${item.description}` : ''}</option>)}</select><small>Select the model Gemini will use for receipt OCR and AI ledger assistant.</small></label><label><span>GEMINI API KEY</span><input type="password" autoComplete="off" spellCheck={false} value={geminiKey} onChange={(event) => { setGeminiKey(event.target.value); setSaved(null) }} placeholder={gemini?.configured ? 'Paste a new key to replace the saved key' : 'Paste your Gemini API key'} /><small>The key is sent over your authenticated connection, encrypted at rest, and never returned to the browser.</small></label><div className="settings-toggle-row"><div><strong>PREVIEW AI TRANSACTIONS</strong><small>{previewTransactions ? 'Gemini opens a filled form for review before saving.' : 'Gemini posts complete requests immediately. It asks when a required detail is unclear.'}</small></div><ActiveToggle active={previewTransactions} label="preview AI transactions" onToggle={() => { setPreviewTransactions((value) => !value); setSaved(null) }} /></div><div className="secure-key-guide"><ShieldDone set="curved" size={23} /><span><strong>SECURE SERVER STORAGE</strong><small>Saving verifies the key with Google, then stores it in Supabase Vault for this signed-in account. Action behavior is saved per workspace.</small></span></div><button className="settings-save" type="submit" disabled={Boolean(saving) || (!geminiKey.trim() && selectedModel === (gemini?.model || DEFAULT_GEMINI_MODEL) && previewTransactions === (settings.geminiTransactionPreview ?? true))}><ShieldDone set="curved" size={19} />{saving === 'gemini' ? 'VERIFYING & SAVING…' : saved === 'gemini' ? 'KEY, MODEL & BEHAVIOR SAVED' : saved === 'gemini_model' ? 'MODEL & BEHAVIOR SAVED' : saved === 'gemini_behavior' ? 'ACTION BEHAVIOR SAVED' : 'SAVE GEMINI SETTINGS'}</button></form>}
